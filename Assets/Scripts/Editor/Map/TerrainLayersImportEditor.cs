...
public class TerrainLayersImportEditor : MonoBehaviour
{
    private Terrain terrain;
    private TerrainLayer[] terrainLayers;

    [MenuItem("Assets/MuOnline/Import terrain layers", false, 1101)]
    private static void ApplyTerrainLayersFromFile()
    {
        string path = AssetDatabase.GetAssetPath(Selection.activeObject);
        if (string.IsNullOrEmpty(path) || !path.EndsWith(".txt"))
        {
            Debug.LogError("❌ Please select a valid TerrainLayers.txt file.");
            return;
        }

        TerrainLayersImportEditor script = new();
