...
public class CameraMovement : MonoBehaviour
{
  ...
  private void UpdateFOV()
  {
    ...
    // If in portrait mode (aspect ratio < 1)
    if (aspectRatio < 1f)
    {
      // Reduce FOV in portrait mode to prevent zooming out
      cam.fieldOfView = BASE_FOV * aspectRatio;
    }
    else
    {
      // Keep standard FOV for landscape and desktop
